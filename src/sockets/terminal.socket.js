...
module.exports = (io, socket) => {
  ...
  socket.on("code:update", ({ projectId, filePath, content }) => {
    socket
      ...
      .emit("code:update", { filePath, content });
  });
  ...
  socket.on("terminal:init", async (payload) => {
    ...
    const isBrowsed = payload.isBrowsed || false;

    if (!projectId || !terminalId) return;

    // Agar already terminal exist karta hai
    if (socket.terminals.has(terminalId)) return;

    try {
      const { ptyProcess, containerName, targetPort } = await spawnTerminal(
        projectId,
        socket,
        isBrowsed,
      );

      socket.terminals.set(terminalId, {
        pty: ptyProcess,
  ...
  socket.on("project:leave", ({ projectId }) => {
    if (projectId && activeFolderUsers[projectId]?.[socket.id]) {
      ...
      io.to(`project_room:${projectId}`).emit(
        "project:users-update",
        Object.values(activeFolderUsers[projectId]),
      );
    }
  ...
  socket.on("disconnect", () => {
    ...
    if (pId && activeFolderUsers[pId]?.[socket.id]) {
      ...
      if (Object.keys(activeFolderUsers[pId]).length === 0) {
        ...
        io.to(`project_room:${pId}`).emit(
          "project:users-update",
          Object.values(activeFolderUsers[pId]),
        );
      }
    }
  });
