...
const uploadLocalProject = async (req, res) => {
  try {
    ...
    const result = await fileService.saveLocalProjectFiles(
      projectId,
      files,
      userId,
      uploadSource,
...
const getFileTree = async (dirPath) => {
  ...
  if (stats.isDirectory()) {
    ...
    const allowedChildren = children.filter(
      (child) => !BANNED_FILES_AND_FOLDERS.includes(child),
    );
...
const getFileTreeHandler = async (req, res) => {
  try {
    ...
    const projectRecord = await Project.findOne({
      where: {
        slug: projectId,
        ...(req?.user?.workspace_id && { workspace_id: req?.user?.workspace_id }), // very newly
      },
    });
...
const saveFileHandler = async (req, res) => {
  ...
  try {
    if (!filePath) {
      return res
        .status(403)
        .json({ error: "Access Denied: Path outside root" });
    }
