...
const fileFilter = (req, file, cb) => {
  const allowedImages = /jpeg|jpg|png|gif|webp/;
  const allowedDocs = /pdf|doc|docx|zip|txt/;
  const allowedAudio = /webm|mp3|wav|m4a|ogg/; // Audio formats allowed for Whisper AI
  
  const ext = path.extname(file.originalname).toLowerCase().replace('.', '');
  
  const isImage = allowedImages.test(ext);
  const isDoc = allowedDocs.test(ext);
  const isAudio = allowedAudio.test(ext);
...
const upload = multer({
  storage: storage,
  limits: { fileSize: 25 * 1024 * 1024 }, // Audio recordings ke liye limit 25MB kar di (Whisper limit)
  fileFilter: fileFilter
});
