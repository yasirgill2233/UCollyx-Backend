...
const spawnTerminal = async (projectId, socket) => {
  ...
  console.log(`🐳 Isolated Sandbox: Container allocated on live port [${targetPort}]`);
...
socket.on("project:join", ({ projectId, username }) => {
  ...
  console.log(`👥 ${socket.username} joined workspace project space: ${projectId}`);
...
socket.on("file:leave", ({ projectId, filePath }) => {
  ...
  if (activeFolderUsers[projectId]?.[filePath]?.[socket.id]) {
    ...
    io.to(roomName).emit("file:active-users-updated", Object.values(activeFolderUsers[projectId][filePath]));
  }
});
...
socket.on("project:leave", ({ projectId }) => {
  ...
  if (activeFolderUsers[projectId]?.[socket.id]) {
    ...
    console.log(`🏃 Explicit Leave: ${leavingUser.name} left project space: ${projectId}`);
...
socket.on("file:leave", ({ projectId, filePath }) => {
  ...
  socket.to(`file_room:${projectId}:${filePath}`).emit("cursor:remove", { socketId: socket.id });
});
